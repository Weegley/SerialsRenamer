...
def getaddrinfo_ipv4_for_selected_hosts(host, port, family=0, type=0, proto=0, flags=0):
    infos = _original_getaddrinfo(host, port, family, type, proto, flags)

    if host in FORCE_IPV4_HOSTS:
        ipv4_infos = [info for info in infos if info[0] == socket.AF_INET]
        return ipv4_infos or infos

    return infos

socket.getaddrinfo = getaddrinfo_ipv4_for_selected_hosts



DEFAULT_KP_API_KEY = "INSERT_YOUR_KINOPOISK_TOKEN_HERE"
...
def extract_year_hint(raw: str) -> Optional[str]:
    years = re.findall(r"\b(19\d{2}|20\d{2}|2100)\b", raw)
...
def sanitize_name_component(name: str) -> str:
    if not name:
        return "_"
    # Keep readable separators in final names whenever possible.
    # Windows-forbidden path separators and colon become a visible dash separator,
    # instead of being silently removed and gluing words together.
    name = name.replace("/", " - ").replace("\\", " - ").replace(":", " - ")
    name = re.sub(r"[\x00-\x1f]", "", name)
    # Other forbidden Windows filename characters are removed conservatively.
    name = re.sub(r'[*?"<>|]', "", name)
...
def sanitize_filename(filename: str) -> str:
    # Preserve only the final real file extension separately.
    #
    # Path.suffixes cannot be used here: every dot in a title is treated as
    # the beginning of another "suffix". Forbidden characters appearing after
    # such a dot could therefore bypass sanitize_name_component().
    p = Path(filename)
    suffix = p.suffix
    stem = filename[:-len(suffix)] if suffix else filename
    safe_stem = sanitize_name_component(stem)
    safe = (safe_stem + suffix).rstrip(" .")
    return safe or "_"
...
def build_initial_search_query(query: str, year: Optional[str]) -> str:
...
def format_kp_series_candidate(item: dict) -> str:
    kp_id = item.get("filmId") or item.get("kinopoiskId")
    name_ru = normalize_spaces(str(item.get("nameRu") or ""))
    name_en = normalize_spaces(str(item.get("nameEn") or ""))
    name_original = normalize_spaces(str(item.get("nameOriginal") or ""))
    name = name_ru or name_en or name_original or "<unnamed>"

    secondary_name = name_en or name_original
    secondary_suffix = ""
    if name_ru and secondary_name and normalize_title_key(secondary_name) != normalize_title_key(name_ru):
        secondary_suffix = f" ({secondary_name})"

    year = item.get("year") or "?"
    typ = item.get("type") or "?"
    return f"{name}{secondary_suffix} ({year}) [{typ}] [kp{kp_id}]"


def format_tmdb_series_candidate(item: dict) -> str:
    tmdb_id = item.get("id")
    name = normalize_spaces(str(item.get("name") or item.get("original_name") or "")) or "<unnamed>"
    original_name = normalize_spaces(str(item.get("original_name") or ""))

    original_suffix = ""
    if original_name and normalize_title_key(original_name) != normalize_title_key(name):
        original_suffix = f" ({original_name})"

    year = tmdb_result_year(item) or "?"
    return f"{name}{original_suffix} ({year}) [TV_SERIES] [tmdb{tmdb_id}]"
...
def interactive_search_loop_tmdb(group: SeriesGroup, tmdb: TMDbClient, initial_query: str, lang_priority: str, mode: str) -> Optional[dict]:
    query = initial_query
    search_year = group.year_hint
    language = "en-US" if lang_priority == "en" else "ru-RU"
    while True:
        display_query = build_initial_search_query(query, search_year)
        print()
        print(f"Source dir   : {group.source_dir.name}")
        print(f"TMDb query   : {display_query}")
        try:
            # TMDb has a dedicated year filter, so the title stays intact.
            # A user-entered query is sent exactly as entered, without this filter.
            results = tmdb.search_tv(query, language=language, year=search_year, page=1)
        except Exception as e:
            print(f"WARNING: TMDb search failed for {display_query}: {e}")
            results = []
        top = results[:10]
        if mode == "smart" and len(top) == 1:
            item = top[0]
            print(f"Auto-selected: {format_tmdb_series_candidate(item)}")
            return item
        if not top:
            print()
            ...
            continue
        print()
        print(f"Found TMDb matches for: {display_query}")
        for idx, item in enumerate(top, start=1):
            print(f"{idx}) {format_tmdb_series_candidate(item)}")
        print("0) exit")
        print("98) skip")
    ...
    for path in root.rglob("*"):
        ...
        if series_dir not in groups:
            groups[series_dir] = SeriesGroup(
                ...
                year_hint=extract_year_hint(series_dir.name),
            )
        stem = path.stem
...
def interactive_search_loop_kp(group: SeriesGroup, kp: KPClient, initial_query: str, lang_priority: str, mode: str) -> Optional[dict]:
    query = build_initial_search_query(initial_query, group.year_hint)
    while True:
        print()
        print(f"Source dir   : {group.source_dir.name}")
        ...
        if mode == "smart" and len(top) == 1 and candidate_matches_lang(top[0], lang_priority):
            item = top[0]
            print(f"Auto-selected: {format_kp_series_candidate(item)}")
            return item
        if not top:
            print()
            print("No matches found.")
            ...
            if kind == "kp":
                ...
                if item:
                    print(f"Selected by kp id: {format_kp_series_candidate(item)}")
                    return item
        ...
        for idx, item in enumerate(top, start=1):
            print(f"{idx}) {format_kp_series_candidate(item)}")
        print("0) exit")
        print("98) skip")
        print("99) retry same search")
        print("Any other input = new search text or kp id")
